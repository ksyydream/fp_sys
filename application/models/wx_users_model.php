...
class Wx_users_model extends MY_Model
{
    ...
    public function edit_foreclosure4s4(){
        ...
        if($borrower_img_SFZ = $this->input->post('borrower_img_SFZ')){
            ...
            if(!@file_get_contents('./upload_files/' . $file_. '/'. $update_['borrower_img_SFZ'])){
                return $this->fun_fail('请上传借款人身份证');
            }
        }else{
            if(!@file_get_contents('./upload_files/' . $file_ . '/' . $f_info_['borrower_img_SFZ'])){
                return $this->fun_fail('请上传借款人身份证');
            }
        }
        if($f_info_['is_mortgage'] == 1){
            if($borrower_spouse_img_SFZ1 = $this->input->post('borrower_spouse_img_SFZ1')){
                ...
                if(!@file_get_contents('./upload_files/' . $file_. '/'. $update_['borrower_spouse_img_SFZ1'])){
                    return $this->fun_fail('请上传配偶身份证');
                }
            }else{
                if(!@file_get_contents('./upload_files/' . $file_ . '/' . $f_info_['borrower_spouse_img_SFZ1'])){
                    return $this->fun_fail('请上传配偶身份证');
                }
            }
        }
